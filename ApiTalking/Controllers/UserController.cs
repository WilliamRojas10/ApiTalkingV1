...
public class UserController : ControllerBase
{
    ...
    {
        _daoUser = daoFactory.CreateDAOUser(); // Obtiene la instancia de IDAOUser
    }
    ...
    public async Task<ActionResult> getUser(int page, int pageSize)
    {
        try
        {
            //  List<EntitiesLibrary.User.User>
            var users = await _daoUser.GetAllUsers();
            //_daoUser.GetUsersPaged(page, pageSize,true);
        ...
        catch (Exception ex)
        {
             return BadRequest(new ErrorResponseDTO
                {
                    sucess = false,
                    message = "No se encontraron usuarios en getUser(): " + ex.Message
                });
        }
    }
    ...
    public async Task<ActionResult> getUserById( int idUser)
     {
        ...
        {
            //  List<EntitiesLibrary.User.User>
            EntitiesLibrary.User.User? user = await _daoUser.GetUserById( idUser );
            //_daoUser.GetUsersPaged(page, pageSize,true);
            ...
            if (user == null)
            {
                ...
                {
                    sucess = false,
                    message = "No se encontro el usuario con el Id: " + idUser
                });
            }
        ...
        catch (Exception ex)
        {
             return BadRequest(new ErrorResponseDTO
                {
                    sucess = false,
                    message = "No se encontraron usuarios en getUserById(): " + ex.Message
                });
        }
    }
